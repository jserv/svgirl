    ...
    struct svgtiny_parse_state *state)
{
    ...
    exc = dom_element_get_attribute(linear, state->interned_gradientTransform,
                                    &attr);
    if (exc == DOM_NO_ERR && attr != NULL) {
        float a = 1, b = 0, c = 0, d = 1, e = 0, f = 0;
        char *s = strndup(dom_string_data(attr), dom_string_byte_length(attr));
        if (s == NULL) {
            dom_string_unref(attr);
    ...
    exc = dom_element_get_elements_by_tag_name(linear, state->interned_stop,
                                               &stops);
    if (exc == DOM_NO_ERR && stops != NULL) {
        ...
        for (stopnr = 0; stopnr < listlen; ++stopnr) {
            ...
            exc =
                dom_nodelist_item(stops, stopnr, (dom_node **) (void *) &stop);
            if (exc != DOM_NO_ERR)
                continue;
            exc =
                dom_element_get_attribute(stop, state->interned_offset, &attr);
            if (exc == DOM_NO_ERR && attr != NULL) {
                                              ...
                                              struct svgtiny_parse_state *state)
{
    ...
    float gradient_x0, gradient_y0, gradient_x1, gradient_y1, gradient_dx,
        gradient_dy;
    float trans[6];
    ...
    gradient_norm_squared =
        gradient_dx * gradient_dx + gradient_dy * gradient_dy;
    ...
    while (a != b) {
        ...
        if (current_stop == 0)
            ...
            shape->fill =
                svgtiny_RGB((int) ((1 - stop_r) * red0 + stop_r * red1),
...
void svgtiny_path_bbox(float *p,
                       unsigned int n,
                       float *x0,
                       float *y0,
                       float *x1,
