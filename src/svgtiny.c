...
static void svgtiny_parse_font_attributes(dom_element *node,
                                          struct svgtiny_parse_state *state);
static void svgtiny_parse_transform_attributes(
    dom_element *node,
    struct svgtiny_parse_state *state);
static svgtiny_code svgtiny_add_path(float *p,
                                     unsigned int n,
                                     struct svgtiny_parse_state *state);
...
static int circle_arc_to_bezier(double start, double extent, double *bzpt)
{
    ...
    controllen =
        4.0 / 3.0 * sin(increment / 2.0) / (1.0 + cos(increment / 2.0));
                            ...
                            double *bzpoints)
{
    ...
    /* compute the transformed centre point */
    compute_transformed_centre_point(largearc == sweep ? -1 : 1, rx, ry, rx_sq,
                                     ry_sq, x1, y1, x1_sq, y1_sq, &cx1, &cy1);

    /* step 3 */
    /* get the untransformed centre point */
    compute_centre_point(start_x, start_y, end_x, end_y, cx1, cy1, radangle,
                         &cx, &cy);
    ...
    scale_rotate_translate_points(rx, ry, radangle, cx, cy, bzsegments * 6,
                                  bzpoints);
...
#define SVGTINY_STRING_ACTION2(s, n)                                     \
    if (dom_string_create_interned((const uint8_t *) #n, strlen(#n),     \
...
svgtiny_code svgtiny_parse(struct svgtiny_diagram *diagram,
                           ...
                           int viewport_height)
{
    return svgtiny_parse_inner(diagram, buffer, size, url, viewport_width,
                               viewport_height);
}
    ...
    do {                                             \
        ...
        if (sscanf(s, " %1[MmLl] %f %f %n", command, &x, &y, &n) == 3) {
            ...
            do {
                ...
                s += n;
            } while (sscanf(s, "%f %f %f %f %f %f %n", &x1, &y1, &x2, &y2, &x,
                            &y, &n) == 6);

            /* shorthand/smooth curveto (S, s) (4 arguments) */
        } else if (sscanf(s, " %1[Ss] %f %f %f %f %n", command, &x2, &y2, &x,
                          &y, &n) == 5) {
            do {
                ALLOC_PATH_ELEMENTS(7);
        ...
        } else if (sscanf(s, " %1[Qq] %f %f %f %f %n", command, &x1, &y1, &x,
                          &y, &n) == 5) {
            do {
                ALLOC_PATH_ELEMENTS(7);
        ...
        } else if (sscanf(s, " %1[Aa] %f %f %f %f %f %f %f %n", command, &rx,
                          &ry, &rotation, &large_arc, &sweep, &x, &y,
                          &n) == 8) {
            do {
                int bzsegments;
                ...
                bzsegments =
                    svgarc_to_bezier(last_x, last_y, x, y, rx, ry, rotation,
                                     large_arc, sweep, bzpoints);
                if (bzsegments == -1) {
                    /* draw a line */
                    ALLOC_PATH_ELEMENTS(3);
                    p[i++] = svgtiny_PATH_LINE;
                ...
                s += n;
            } while (sscanf(s, "%f %f %f %f %f %f %f %n", &rx, &ry, &rotation,
                                       ...
                                       float *height)
{
    ...
    *height = state.viewport_height;
    ...
    if (exc == DOM_NO_ERR && attr != NULL) {
        *width = svgtiny_parse_length(attr, state.viewport_width, state);
        dom_string_unref(attr);
    }

    exc = dom_element_get_attribute(node, state.interned_height, &attr);
    if (exc == DOM_NO_ERR && attr != NULL) {
        *height = svgtiny_parse_length(attr, state.viewport_height, state);
        dom_string_unref(attr);
    }
}
                                    ...
                                    struct svgtiny_parse_state *state)
{
    ...
    exc =
        dom_element_get_attribute(node, state->interned_stroke_opacity, &attr);
    if (exc == DOM_NO_ERR && attr != NULL) {
        float f = svgtiny_parse_length(attr, 1, *state);
        if (0.0f <= f && f <= 1.0f) {
            int alpha = f * 0xff;
    ...
    if (exc == DOM_NO_ERR && attr != NULL) {
        state->stroke_width =
            svgtiny_parse_length(attr, state->viewport_width, *state);
        dom_string_unref(attr);
    }
    ...
    if (exc == DOM_NO_ERR && attr != NULL) {
        char *style =
            strndup(dom_string_data(attr), dom_string_byte_length(attr));
        const char *s;
        char *value;
        ...
        if ((s = strstr(style, "stroke-width:"))) {
            ...
            value = strndup(s, strcspn(s, "; "));
            state->stroke_width =
                _svgtiny_parse_length(value, state->viewport_width, *state);
            free(value);
        }
                                        ...
                                        struct svgtiny_parse_state *state)
{
    ...
    if (exc == DOM_NO_ERR && attr != NULL) {
        ...
        svgtiny_parse_transform(transform, &state->ctm.a, &state->ctm.b,
                                &state->ctm.c, &state->ctm.d, &state->ctm.e,
                                &state->ctm.f);
        free(transform);
        dom_string_unref(attr);
    }
                             ...
                             float *mf)
{
    ...
    while (*s) {
        ...
        if ((sscanf(s, " matrix (%f %f %f %f %f %f ) %n", &a, &b, &c, &d, &e,
            ...
            d = a;
        else if ((sscanf(s, " rotate (%f %f %f ) %n", &angle, &x, &y, &n) ==
                  3) &&
                 (n > 0)) {
            ...
            angle = angle / 180 * M_PI;
            c = tan(angle);
        } else if ((sscanf(s, " skewY (%f ) %n", &angle, &n) == 1) && (n > 0)) {
            angle = angle / 180 * M_PI;
            b = tan(angle);
        } else
            break;
...
svgtiny_code svgtiny_add_path(float *p,
                              unsigned int n,
                              struct svgtiny_parse_state *state)
{
